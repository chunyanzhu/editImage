<template>
	<div :class="['selection', {selection_down: showMenu, 'disabled': disabled}]" :style="{'zIndex': zIndex}">
		...
		<i :class="['dropdown_icon', {'up': showMenu}]" @click.stop="!disabled ? (showMenu = !showMenu) : ''" ref="downIcon"></i>
		<div class="text" ref="text" @click.stop="!disabled ? (showMenu = true) : ''">{{getText()}}</div>
